// Modified from the rust-crypto::bcrypt_pbkdf
...
fn bcrypt_hash(hpass: &[u8], hsalt: &[u8], output: &mut [u8; 32]) {
    ...
    for _ in 0..64 {
        bf.bc_expand_key(hsalt);
        bf.bc_expand_key(hpass);
    }

    let mut buf = [0u32; 8];
    BigEndian::read_u32_into(b"OxychromaticBlowfishSwatDynamite", &mut buf);

    for i in (0..8).step_by(2) {
        for _ in 0..64 {
            let (l, r) = bf.bc_encrypt(buf[i], buf[i + 1]);
            buf[i] = l;
            buf[i + 1] = r;
        }
    }
...
pub fn bcrypt_pbkdf(
    ...
    if output.is_empty() || output.len() > 1024 {
        return Err(ErrorKind::InvalidLength.into());
    }
